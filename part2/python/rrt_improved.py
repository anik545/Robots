...
def check_arc(node_a, node_b, grid):
    centre, r = find_circle(node_a, node_b)

    initial = node_a.position - centre
    final = node_b.position - centre

    clockwise = True if node_b.yaw - node_a.yaw < 0 else False
    arc_angle = np.abs(node_b.yaw - node_a.yaw)
    arc_angle = angle_between(initial, final)
    arc_length = r * arc_angle
    step = 0.004
    samples = np.ceil(arc_length/step)

    # print("check arc", node_a.position, node_b.position)
    # print("arc angle", arc_angle, 'rads', np.degrees(arc_angle), 'deg')
    # print("initial", initial)
    # print("final", final)
    # print("centre", centre)
    # print("arc_length", arc_length)
    for theta in np.linspace(0, arc_angle, samples):
        t = -theta if clockwise else theta
        v = rotate(initial, t) + centre

        # print("checking: ", v, "theta: ", t, clockwise)
