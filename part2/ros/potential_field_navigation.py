...
def get_relative_position(absolute_pose, absolute_position):
    ...
    relative_position[X] = (absolute_position[X] - absolute_pose[X])
    relative_position[Y] = (absolute_position[Y] - absolute_pose[Y])
...
def get_velocity(point_position, goal_position, obstacle_position):
    v_goal = potential_field.get_velocity_to_reach_goal(
        point_position, goal_position)
    ...
    while not rospy.is_shutdown():
        ...
        if USE_RELATIVE_POSITIONS:
            point_position = get_relative_position(
                groundtruth.pose, absolute_point_position)
            goal_position = get_relative_position(
                groundtruth.pose, GOAL_POSITION)
            obstacle_position = get_relative_position(
                groundtruth.pose, CYLINDER_POSITION)
        ...
        pose_history.append(np.concatenate(
            [groundtruth.pose, absolute_point_position], axis=0))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Runs potential field navigation')
